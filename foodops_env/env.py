...
class FoodOpsEnv(gym.Env):  # type: ignore[misc]
    """Compact deterministic environment for three ambiguous payout-drop scenarios."""
    ...
    def reset(self, seed: int | None = None, **_: Any) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        self.anomaly.template.inject_fn(self.world, self.anomaly.params)
        self.world["active_pair"] = (self.anomaly.params["brand"], self.anomaly.params["channel"])
    ...
    def render(self) -> str:
        ...
        lines = [
            ...
            f"Scenario: {self.anomaly.scenario_id}",
    ...
    def _observation(self) -> dict[str, Any]:
        ...
        brand = self.anomaly.params["brand"]
        channel = self.anomaly.params["channel"]
    ...
    def _tool_force_menu_sync(self, brand: str | None, channel: str | None) -> dict[str, Any]:
        ...
        return {
            ...
            "brand": brand,
            "channel": channel,
            "queued_at": self.world["now"].isoformat(),
        }
    ...
    def _aggregate_pairs(self, source: dict[tuple[str, str], dict[str, Any]], brand: str | None, channel: str | None) -> dict[str, Any]:
        matching = [
            row
            for (row_brand, row_channel), row in source.items()
        ...
        refund_amount = round(sum(row["refund_amount"] for row in matching), 2)
