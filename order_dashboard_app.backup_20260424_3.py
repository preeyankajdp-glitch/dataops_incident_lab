...
def build_status_strip(scenario_name: str, metric_date: str) -> str:
    ...
    kind_labels = {
        ...
        "kpi_reasoning": ("KPI reasoning", "pill-kpi"),
        "data_issue": ("Data / config issue", "pill-data"),
        "compound": ("Compound", "pill-compound"),
    }
...
def tool_get_brand_channel_config(brand_name: str | None = None, channel_name: str | None = None) -> dict[str, Any]:
    filters = []
    params: list[Any] = []
    if brand_name:
...
def tool_inspect_pipeline_logs(limit: int = 8) -> dict[str, Any]:
    if not LOG_PATH.exists():
        return {"runs": []}
    payload = json.loads(LOG_PATH.read_text())
...
def llm_assistant(message: str, history: list[dict[str, str]], focus_date: str) -> tuple[list[dict[str, str]], str, str]:
    ...
    snapshot = tool_get_metric_snapshot(metric_date, brand_name, channel_name)
    config = tool_get_brand_channel_config(brand_name, channel_name)
    freshness_orders = tool_get_table_freshness("orders")
    freshness_agg = tool_get_table_freshness("channel_daily_metrics")
    logs = tool_inspect_pipeline_logs(limit=8)
    kpis = tool_list_kpis()

    trace.extend(
        [
            {"tool": "get_metric_snapshot", "arguments": {"metric_date": metric_date, "brand_name": brand_name, "channel_name": channel_name}, "result": snapshot},
            {"tool": "get_brand_channel_config", "arguments": {"brand_name": brand_name, "channel_name": channel_name}, "result": config},
            {"tool": "get_table_freshness", "arguments": {"table_name": "orders"}, "result": freshness_orders},
            {"tool": "get_table_freshness", "arguments": {"table_name": "channel_daily_metrics"}, "result": freshness_agg},
            {"tool": "inspect_pipeline_logs", "arguments": {"limit": 8}, "result": logs},
        ]
    )

    context = {
        "question": message,
        "focus_date": metric_date,
        "brand_name": brand_name,
        "channel_name": channel_name,
        "snapshot": snapshot,
        "config": config,
        "freshness": {"orders": freshness_orders, "channel_daily_metrics": freshness_agg},
        "pipeline_logs": logs,
        "kpis": kpis,
    }

    system_prompt = (
        "You are an operations analyst for a food delivery brand x channel network. "
        "Explain KPI movement in plain business language. Be concrete, concise, and honest about uncertainty. "
        "Use the evidence provided; do not invent new facts. "
        "If the issue looks like a business/config problem rather than a pipeline failure, say that clearly. "
        "If both are true, say both."
    )
    user_prompt = (
    ...
    try:
        ...
        latest_result = json.dumps(snapshot, indent=2, default=str)
        # Stash the trace so submit_incident_report can score against it
        global _LATEST_TRACE_FOR_SCORING
        _LATEST_TRACE_FOR_SCORING = trace
        return history, answer, json.dumps(trace, indent=2, default=str), latest_result
    except Exception as exc:
        ...
        labeled_answer = (
            "⚠️ **Heuristic fallback** (OpenAI API unavailable — quota, key, or network). "
...
def build_demo() -> gr.Blocks:
    ...
    with gr.Blocks(title="FoodOps Incident Lab") as demo:
        ...
        with gr.Tab("Dashboard"):
            ...
            # --- Incident Report submission & reward scoring ---
            gr.HTML("<div style='border-top:1px solid var(--border); margin-top:18px; padding-top:14px;'></div>")
            gr.Markdown("### Submit an incident report")
            gr.Markdown(
                "Once you (or the assistant) have diagnosed the issue, submit a report. "
                "The environment scores it against the hidden ground truth — this is the "
                "benchmark signal a trained agent would receive."
            )
            with gr.Row():
                report_category = gr.Dropdown(
                    label="Root cause category",
                    choices=ROOT_CAUSE_CATEGORIES,
                    value="business_config_drift",
                    scale=2,
                )
                report_kpis = gr.CheckboxGroup(
                    label="Affected KPIs",
                    choices=list(KPI_DEFINITIONS.keys()),
                    value=[],
                    scale=3,
                )
            with gr.Row():
                report_cause = gr.Textbox(
                    label="Root cause (free text)",
                    placeholder="e.g. promo_active flag stuck on for Behrouz × Swiggy after campaign ended",
                    scale=4,
                )
                report_fix = gr.Textbox(
                    label="Proposed fix",
                    placeholder="e.g. flip promo_active=false in channel_brand_config",
                    scale=4,
                )
                submit_report_btn = gr.Button("Submit & Score", variant="primary", scale=1)
            reward_card_html = gr.HTML(
                "<div class='reward-empty'>No report submitted yet. Fill in the fields above "
                "and click <strong>Submit &amp; Score</strong> to get an episode reward.</div>"
            )
        ...
        with gr.Tab("Scenario Studio"):
            gr.Markdown(
                "### Simulate a real-world incident on the pipeline\n"
                "The app streams orders into a raw table, which is rolled up into an "
                "aggregate, which powers the dashboard. Pick a scenario and watch which "
                "part of this pipeline breaks. Then head to the Dashboard and ask the "
                "assistant to diagnose it."
            )
            with gr.Row():
        ...
        overview_date.change(
            ...
            outputs=[
                ...
                spotlight_table, scenario_actions, table_counts, focus_date,
            ],
        )
        ...
        reset_btn.click(
            ...
            outputs=[
                ...
                storybook_html, active_scenario,
        ...
        load_btn.click(
            ...
            outputs=[
                ...
                config_table, spotlight_table, scenario_actions, status_bar,
        ...
        def handle_rebuild(metric_date: str):
            ...
            return (
                ...
                get_table_counts(), pipeline_html,
            )
        ...
        def handle_submit_report(category: str, kpis: list[str], cause: str, fix: str):
            """Submit an incident report and return the reward card."""
            if not cause.strip():
                return (
                    "<div class='reward-empty' style='color:var(--signal-bad);'>"
                    "Please provide a root-cause description before submitting.</div>"
                )
            result = tool_submit_incident_report(
                root_cause=cause,
                fix=fix or "(none specified)",
                root_cause_category=category,
                affected_kpis=kpis or [],
            )
            reward = result.get("reward", {})
            return render_reward_card(reward)

        submit_report_btn.click(
            handle_submit_report,
            inputs=[report_category, report_kpis, report_cause, report_fix],
            outputs=[reward_card_html],
        )
