...
def tool_submit_incident_report(root_cause: str, fix: str) -> dict[str, Any]:
    payload = {
        "status": "submitted",
    ...
    write_log("submit_incident_report", "Assistant submitted an incident summary.", payload)
    return payload
...
def llm_assistant(message: str, history: list[dict[str, str]], focus_date: str) -> tuple[list[dict[str, str]], str, str]:
    ...
    try:
        ...
        return fallback_history, fallback_answer, json.dumps(merged_trace, indent=2, default=str), latest_result
...
def build_demo() -> gr.Blocks:
    ...
    with gr.Blocks(title="FoodOps Incident Lab") as demo:
        ...
        with gr.Tab("Data Explorer"):
            ...
            with gr.Row():
                table_sql = gr.Code(label="Create / Aggregate SQL", language="sql")
                sql_runner = gr.Code(label="Editable SQL", language="sql")
            execute_sql_btn = gr.Button("Run SQL", variant="primary")
        ...
        def inspect_table(table_name_value: str):
            ...
            return (
                get_table_sql(table_name_value),
                get_table_sql(table_name_value),
        ...
        def execute_sql(query: str):
            ...
            return "SQL executed.", gr.update(headers=columns, value=rows), get_pipeline_log_rows(), get_table_counts()

        execute_sql_btn.click(
            execute_sql,
            inputs=[sql_runner],
            outputs=[sql_status, table_preview, pipeline_logs, table_counts],
        )
        ...
        sql_text, _, preview_update, pipeline_log_rows, count_rows = inspect_table("orders")
        table_sql.value = sql_text
        sql_runner.value = sql_text
